...
np.random.seed(1)
font = fm.FontProperties(fname='C:/Windows/Fonts/SimSun-ExtB Regular')
matplotlib.rcParams['figure.figsize'] = (10.0, 8.0)
def sigmoid(input_sum):
    """函数：激活函数
       参数：
       imput_sum 输入即表示神经元的加权和
       返回：
       output:激活后的输出
       input_sum: 把输入缓存起来返回"""
...
def sigmoid_back_propagation(derror_wrt_output,input_sum):
    """函数：
            ...
            其中dOut/dIn就是激活函数的导数 dy=y(1-y)
            dE/dOut 误差对神经输出的偏导
            输入：
            derror_wrt_output:误差关于神经元输出的偏导：dE/dyj=1/2(d(expect_to_output-output)**2/d output=-(expect_to_output-output)
            input_sum 输入加权和
            返回：
            derror_wrt_dinput: 误差关于神经元输入的偏导"""
    output = 1.0 / (1 + np.exp(-input_sum))
...
def relu_back_propagation(derror_wrt_output,input_sum):
    ...
    误差对神经输出的偏导
    输入：
    derror_wrt_output: 误差关于神经元输出的偏导：dE / dyj = 1 / 2(d(expect_to_output - output) ** 2 / d
    output = -(expect_to_output - output)
    input_sum
    输入加权和
    返回：
    derror_wrt_dinput: 误差关于神经元输入的偏导
    """
    derror_wrt_dinput=np.array(derror_wrt_output,copy=True)
    derror_wrt_dinput[input_sum<=0]=0
    return derror_wrt_dinput
def tanh(input_sum):
    """函数：
           激活函数 tanh
...
class NeuralNetwork:
    ...
    def set_num_iteration(self,num_iterations):
        """设置迭代次数"""
    ...
    def init_w_and_b(self):
        ...
        初始化神经网络所有参数
        输入：
            layers_structure:神经网络的结构，例如layers_strcuture: 神经网络的结构，例如[2,4,3,1]，4层结构:
    ...
    #    b1 = array([ 0.,  0.,  0.,  0.])
        for l in range(1,self.layers_num):
            self.w["w"+str(l)]=np.random.randn(self.layers_structure[l],self.layers_structure[l-1])/np.sqrt(self.layers_strcuture[l-1])
    ...
    def forward_propagation(self,x):
        ...
        神经网络的正向传播
          ...
          cathes:正向传播过程中缓存没一个网络的信息：（x,w,b,iiput_sum --cathes
       """
        cathes=[]
        #作为输入层
        output_prev=x
        ...
        for l in range(1,L):
            ...
            output_prev,cathe=self.layer_activation_forward(input_cur,self.w["w"+str(l)],self.b["b"+str(l)],"tanh")
            cathes.append(cathe)
        output,cathe=self.layer_activation_forward(output_prev,self.w["w"+str(L)],self.b["b"+str(L)],"sigmoid")
        cathes.append(cathe)
        return output,cathes
    ...
    def layer_activation_backward(self,derror_wrt_output,cache,activation_choose):
        """
        函数 ：网络层的反向传播
        :param derror_wrt_output:误差关于输出的偏导
        :param cache: 网络层的缓存信息（x,w,b,input_sum)
        :param activation_choose: 选择激活函数
        :return: derror_wrt_output_prev: 反向传播到上一层的误差关于输出的梯度
        ...
        m=output_prev.shape[1]   #m是输入的样本数量
        #误差关于权值w的偏导数
        ...
        # 为反向传播到上一层提供误差传递
        derror_wrt_ouput_prev=np.dot(w.T,derror_wrt_dinput)
        return derror_wrt_ouput_prev,derror_wrt_dw,derror_wrt_db
    def back_propagation(self,output,caches):
        """
        函数：神经网络的反向传播
        :param output: 神经网络输出
        :param caches: 所有网络层的缓存参数信息[(x,w,b,input_sum]
        :return: grads: 返回当前迭代的梯度 信息
        """
        grads=dict()
        L=self.param_layers_num
        output=output.reshape(output.shape)   #把输出层输出重构成和期望输出一样的结构
    ...
    def training_model(self):
        ...
        if False:
            ...
            plt.title(u'学习率='+str(self.learning_rate))
            plt.show()
        return self.w,self.b
    def predict_by_modle(self,x):
        """使用训练好的模型（即最后求得的w,b参数）来决策输入样本的结果 """
